...
def compute_bus_conditions(bus, state, t_idx, delta_t): 
    ...
    elif len(bus.fuel_cell_stacks) >  1:  
        bus_conditions = state.conditions.energy[bus.tag] 
        if bus.fuel_cell_stack_electric_configuration == 'Series':
            fc_conditions                               = [bus_conditions.fuel_cell_stacks[fc.tag] for fc in bus.fuel_cell_stacks]
            bus_conditions.voltage_open_circuit[t_idx]  = sum(fc.voltage_open_circuit[t_idx] for fc in fc_conditions)
            bus_conditions.voltage_under_load[t_idx]    = sum(fc.voltage_under_load[t_idx] for fc in fc_conditions) 
    
        elif bus.battery_module_electric_configuration == 'Parallel':
            fc_conditions                               = [bus_conditions.fuel_cell_stacks[fc.tag] for fc in bus.fuel_cell_stacks] 
            bus_conditions.voltage_open_circuit[t_idx]  = fc_conditions[-1].voltage_open_circuit[t_idx]
            bus_conditions.voltage_under_load[t_idx]    = fc_conditions[-1].voltage_under_load[t_idx]              
