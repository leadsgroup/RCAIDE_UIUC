...
def supersonic(Z,XSQ1,RO1,XSQ2,RO2,XTY,T,B2,ZSQ,TOLSQ,TOL,TOLSQ2,X1,Y1,X2,Y2,RTV1,RTV2,CUTOFF,CHORD,RNMAX,n_cp,TE_ind, LE_ind):
    ...
    bool1           = np.ones(shape,dtype=np.bool) # not using any alias to ensure compatibilty across all numpy versions
    bool1[:,X1<TOL] = False
    bool1[RAD1==0.] = False
    RAD1[:,X1<TOL]  = 0.0
    ...
    bool2           = np.ones(shape,dtype=np.bool) # not using any alias to ensure compatibilty across all numpy versions
    bool2[:,X2<TOL] = False
    bool2[RAD2==0.] = False
    RAD2[:,X2<TOL]  = 0.0
    ...
    # Setup masks
    F_mask = np.ones((n_mach,size),dtype=np.bool) # not using any alias to ensure compatibilty across all numpy versions
    A_mask = np.ones((n_mach,size),dtype=np.bool) # not using any alias to ensure compatibilty across all numpy versions
    F_mask[:,TE_ind] = False
    A_mask[:,LE_ind] = False
