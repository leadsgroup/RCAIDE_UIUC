...
# ----------------------------------------------------------------------------------------------------------------------         
def ground_proximity_effect (Velocity_mixed,sound_ambient,theta_m,height,Diameter_mixed,frequency):
    ...
    GPROX_m = (5*Velocity_mixed/sound_ambient)*np.exp(-(9*(theta_m/np.pi)-6.75)**2- \
        ((height/Diameter_mixed)-2.5)**2)*(1+(np.sin((np.pi*height*frequency/sound_ambient)-np.pi/2))**2)/ \
        (2+np.abs((height*frequency/sound_ambient)-1))
