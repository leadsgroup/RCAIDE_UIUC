...
# ----------------------------------------------------------------------------------------------------------------------    
def compute_PMSM_motor_performance(motor):
  
    I_turn         = motor.I_tot/motor.N                                               # [A]            current in each turn
    omega          = motor.Kv*(motor.V - motor.I_tot*motor.R)* (2 * np.pi / 60)        # [RPM -> rad/s] rotor angular velocity
    ...
    A_sign         = (motor.k_w*motor.I_tot)/(np.pi*motor.D_in)                        # [-]            stator electrical loading (Eq.2)        
